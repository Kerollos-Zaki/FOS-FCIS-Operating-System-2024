...
void* create_user_kern_stack(uint32* ptr_user_page_directory)
{
	...
	//cprintf("I'm in create_user_kern_stack... \n");
	//uint32 stackSize = KERNEL_STACK_SIZE;

	//uint32 stackTop = USTACKTOP - KERNEL_STACK_SIZE;
	//uint32 stackBottom = USTACKTOP - PAGE_SIZE;

	// Allocate memory ll kernel stack
	/*uint32 stackBottom = KERN_STACK_TOP;
	uint32 pages_bytes = 0;
	while(pages_bytes < KERNEL_STACK_SIZE){
		cprintf("I'm in create_user_kern_stack 2\n");
		struct FrameInfo* frame_info = NULL;
		int result = allocate_frame(&frame_info);

		if (result != 0) {
			//return al panic law mfe3 memory
			panic("create_user_kern_stack(): Failed to allocate frame for kernel stack");
		}
		map_frame(ptr_user_page_directory, frame_info, KERN_STACK_TOP - pages_bytes,PERM_PRESENT | PERM_WRITEABLE);
		pages_bytes = pages_bytes + PAGE_SIZE;
	}
	cprintf("I'm after while loop 2\n");
	*/
	void *start_addr_stack = kmalloc(KERNEL_STACK_SIZE); // it returns the address of the first byte of the first page - Bottom Stack page

	if(start_addr_stack == NULL){
		panic("");
		return NULL;
	}

	// Mark al page as NOT PRESENT (al gaurd page)
	//uint32 guardPageAddress = stackBottom;
	//unmap_frame(ptr_user_page_directory, KERN_STACK_TOP);
	pt_set_page_permissions(ptr_page_directory,(uint32)start_addr_stack,0,PERM_PRESENT); // bottom stack

	// Return al top of the stack akno stack pointer
	return start_addr_stack;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	//cprintf("It is called in Dynamic allocator user heap\n");
	e->USERH_START = (uint32 *) daStart;
	e->USERH_BRK = (uint32 *) daStart; // for now not sure
	e->USERH_HARD_LIMIT = (uint32 *) daLimit;
	initialize_dynamic_allocator(daStart,0);

	// list >> user_environment.h
	// initialization > list (DONE)
	// malloc() (user side) loop
	// free()  >> remove but still sorted or just increment no of consecutive pages if the previous page is free
	// memlayout.h
	// mmu.h
	// uheap.h
