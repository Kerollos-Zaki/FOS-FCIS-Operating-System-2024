...
{
	// sum 5 6 : all arguments
	// sum : args[0] , 5 : args[1] , 6 : args[2]
	//TODO: [PROJECT'24.MS1 - #01] [1] PLAY WITH CODE! - process_command
	LIST_INIT(&foundCommands); //reinitialize the list , delete what inside it , size = 0 , head = NULL, tail = NULL
	...
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
	{
		if (strcmp(arguments[0], commands[i].name) == 0)
		{
			if((number_of_arguments-1 == commands[i].num_of_args)||(commands[i].num_of_args == -1 && number_of_arguments-1 >= 1)){
				return i;
			}else{ // command is found but invalid number of arguments
				// add that command in list
	...
	// if got here means no return statement found above , means Command not found , we don't have to create a flag found or not (if flag == 0)
	// command is not found BUT its chars are subsequence-matched with one or more commands

	uint32 found_subs = 0;
	for(int i = 0; i < NUM_OF_COMMANDS; i++){


		// Subsequence using two pointers technique

		// cwt // argument[0]
		// cabqwjjt // command[i].name

		uint32 ptr1=0;
		uint32 ptr2=0;
		while(ptr1 < strlen(arguments[0])&& ptr2 < strlen(commands[i].name)){
			if(arguments[0][ptr1] == commands[i].name[ptr2]){
				ptr1++;
			}
			ptr2++;
		}
		if(ptr1 == strlen(arguments[0])){ // that means it's subsequence
			if(found_subs != 1){
				found_subs = 1;
			}
			// check the size of the list to add the first element as head, if more than or equal to 1 , INSERT AFTER NOT INSERT HEAD
			if(LIST_SIZE(&foundCommands) == 0){ // list is empty
				LIST_INSERT_HEAD(&foundCommands, &commands[i]);
			}else{
