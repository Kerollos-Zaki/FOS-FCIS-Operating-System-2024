...
//		cprintf("*	IRQ14 (Primary ATA Hard Disk): is Enabled\n");
	...
	cprintf("Size of block AFTER MALLOC: %u \n",*((uint32*)((uint32)LIST_FIRST(&freeBlocksList) - 4)));
	cprintf("Size of free Block List AFTER: %u \n",freeBlocksList.size);
	cprintf("AFTER KMALLOC() \n");
	cprintf("address of kmalloc: %p \n",first_page);
	...
	cprintf("AFTER initialization no_consecutive_pages\n");
	LIST_INSERT_HEAD(&(freePagesList_KH),first_page);
	cprintf("AFTER Inserting into LIST_HEAD \n");*/
