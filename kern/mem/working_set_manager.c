...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	// Allocate memory ll new WorkingSetElement
	// kmalloc need to be implemented mn al kernel heap
	//cprintf("I'm in env_page_ws_list_create_element \n");
	struct WorkingSetElement* new_element = (struct WorkingSetElement*) kmalloc(sizeof(struct WorkingSetElement));
	//struct WorkingSetElement* new_element = (struct WorkingSetElement*)virtual_address;
	// Check lw al allocation failed
	if (new_element == NULL) {
		panic("env_page_ws_list_create_element: failed to allocate memory for WorkingSetElement");
		return NULL;
	}
	// Initialize al virtual_address
	//new_element->virtual_address = ROUNDDOWN(virtual_address, PAGE_SIZE);
	virtual_address = ROUNDDOWN(virtual_address, PAGE_SIZE); // 3shan lw edany address bta3 byte fy nos elpage fa arg3 l base addr bta3 page (awl byte mn page)
	new_element->virtual_address = virtual_address;

	// Initialize other fields of the WorkingSetElement as needed
	new_element->time_stamp = 0;          // Set initial timestamp ll last access or modification time of a page
	new_element->sweeps_counter = 0;  //how many times a page has been considered for replacement
	//cprintf("BEFORE \n");
	//env_page_ws_print(e);
	//cprintf("Max size in Process: %u \n", e->page_WS_max_size);
	//LIST_INSERT_HEAD(&(e->page_WS_list), new_element);  >>> false gives error
	//cprintf("AFTER \n");
	//env_page_ws_print(e);
	//e->page_last_WS_element = new_element;
	return new_element; // Return pointer ll created object
