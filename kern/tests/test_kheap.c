...
int test_fastfirstfit()
{
	...
	{
		ptr_fast_allocations[i] = kmalloc(allocSize); // fy kol mra by3ml allocate b 8KB (2 pages)
	}
	cprintf("\n2\n");
	...
	kfree(ptr_fast_allocations[0]);		// Hole 1 = allocSize , LIST IS EMPTY
	//cprintf("----------------------------- \n"); // <<<<<<<<<<<<<<<<<<<
...
int test_kfree_bestfirstfit()
{
	...
	cprintf("\n4. Allocate after kfree [15%]\n");
	{
		...
		if (ptr_allocations[9] != (void *)((uint32)ptr_allocations[2] + sizeof(struct FreePagesInfo) + 2*sizeof(int))) // 16 bytes for struct object + 8 bytes for header and footer 2*sizeof(int) = 2*4 bytes = 8 bytes
	...
	cprintf("\n7. Allocate after kfree ALL [30%]\n");
	{
		...
		/*[DYNAMIC ALLOCATOR] Allocate in merged freed space*/
		{
			...
			if (ptr_allocations[11] != (void *)((uint32)ptr_allocations[2] + sizeof(struct FreePagesInfo) + 2*sizeof(int)))  // <<<<<<<<<<<
			...
			expected = (void *)((uint32)ptr_allocations[2] + sizeof(struct FreePagesInfo) + 2*sizeof(int) + 1*kilo + 2*sizeof(int)); // <<<<<<<
			...
			expected = (void *)((uint32)ptr_allocations[2] + sizeof(struct FreePagesInfo) + 2*sizeof(int) + 1*kilo + 2*sizeof(int) + 2*kilo + 2*sizeof(int)); // <<<<<<<
