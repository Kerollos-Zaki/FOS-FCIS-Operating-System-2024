...
void sys_free_user_mem(uint32 virtual_address, uint32 size)
{
	...
	if (virtual_address == 0|| virtual_address < USER_HEAP_START || virtual_address >= USER_HEAP_MAX|| virtual_address + size >= USER_HEAP_MAX){
		env_exit();
		return;
	}
...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size) // user side
{
	//TODO: [PROJECT'24.MS1 - #03] [2] SYSTEM CALLS - Params Validation
	if (virtual_address == 0|| virtual_address < USER_HEAP_START || virtual_address >= USER_HEAP_MAX|| virtual_address + size >= USER_HEAP_MAX){
		env_exit();
	...
	allocate_user_mem(cur_env, virtual_address, size); // kernel side
	return;
}
...
void *sys_getConsecutiveFreePages(struct Env *env,uint32 size){
	...
	return getConsecutiveFreePages(env, size);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_get_consecutive_free_pages:
		return (uint32)sys_getConsecutiveFreePages((struct Env *)a1,a2);
		//return 0;
		break;
